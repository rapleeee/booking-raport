...
@section('header', 'Kelola Wali Kelas')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="col-span-1">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 class="text-sm font-bold text-gray-800 mb-4">Tambah Wali Kelas</h3>
            <form action="{{ route('admin.walikelas.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Nama</label>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="pin" required maxlength="6"
                        class="w-full bg-gray-50 border border-gray-200 text-gray-800 rounded-xl px-4 py-2.5 outline-none focus:border-[#4369a8] transition-colors">
                    @error('pin')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
    ...
    <div class="col-span-1 md:col-span-2">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="text-sm font-bold text-gray-800">Daftar Wali Kelas</h3>
            </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 font-medium text-gray-800">{{ $wali->nama }}</td>
                            <td class="px-4 py-4 text-gray-500">{{ $wali->pin }}</td>
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.walikelas.edit', $wali->id) }}" class="p-1.5 rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-100 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </a>
                                    <form action="{{ route('admin.walikelas.destroy', $wali->id) }}" method="POST" class="form-delete">
                                        @csrf @method('DELETE')
